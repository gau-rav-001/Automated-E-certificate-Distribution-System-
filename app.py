...
/* ── Streamlit block container — fluid padding ── */
.block-container {
...
[data-testid="stSidebar"] {
  background: linear-gradient(160deg, #120f08 0%, #0d0a05 100%) !important;
...
/* ── 3-D cert card — scales down on small screens ── */
.cert-3d-wrap {
  flex-shrink: 0;
  perspective: 900px;
  /* centre on mobile when stacked */
  margin: 0 auto;
}
.cert-3d {
  width: clamp(140px, 22vw, 220px);
  height: clamp(100px, 15vw, 155px);
  ...
  box-shadow:
    ...
    inset 0 1px 0 rgba(245,200,66,0.18);
}
.cert-3d::before {
  content: '';
...
.logo-badge {
  ...
  border-radius: 50px;
  padding: 0.28rem clamp(0.6rem, 2vw, 1rem);
...
.hero-stat {
  ...
  border-radius: 50px;
  padding: clamp(0.28rem, 1vw, 0.4rem) clamp(0.6rem, 1.5vw, 1rem);
...
.gold-divider::after {
  ...
  background: var(--ink); padding: 0 0.7rem;
...
/* disable heavy 3-D tilt on touch devices — prevents jank */
@media (hover: hover) {
  .step-card:hover {
...
.stButton > button:disabled {
  ...
  color: rgba(253,246,227,0.2) !important;
...
[data-testid="stSlider"] [role="slider"] {
  ...
  box-shadow: 0 0 10px rgba(245,200,66,0.45) !important;
}
...
══════════════════════════════════════════ */
.stat-row {
  display: flex; gap: clamp(0.5rem,1.5vw,0.8rem);
...
/* ══════════════════════════════════════════
   SIDEBAR LOGO
══════════════════════════════════════════ */
.sidebar-logo-wrap { padding: 1rem 0 0.4rem; text-align: center; }
.sidebar-logo-icon {
...
.sidebar-sep {
  width:100%; height:1px;
  background: linear-gradient(90deg, transparent, rgba(245,200,66,0.28), transparent);
  margin: 1rem 0;
}
.sidebar-section {
  font-family: 'Space Mono', monospace;
...
@media (max-width: 1023px) {
  ...
  .hero-stats .hero-stat { font-size: 0.75rem; }
}
...
@media (max-width: 480px) {
  ...
  .block-container {
    padding: 0.6rem 0.7rem 3rem !important;
  }

  /* hero stacks vertically, cert card centred */
  .hero-wrapper {
  ...
  /* 2-col stat pills on mobile */
  .stat-row { gap: 0.4rem; }
  .stat-pill { font-size: 0.72rem; padding: 0.32rem 0.6rem; }
  ...
  /* Streamlit columns collapse to single col on mobile */
  /* (Streamlit already does this, we just reinforce) */
  [data-testid="column"] {
    width: 100% !important;
  ...
  /* number inputs: 3-across too cramped — let them stack */
  .stNumberInput { width: 100% !important; }

  /* log box shorter on mobile */
  .log-box { max-height: 180px; }

  /* hide decorative stars on tiny screens */
  [data-testid="stAppViewContainer"]::before,
  [data-testid="stAppViewContainer"]::after { display: none; }

  /* success banner */
  .success-banner { padding: 1.4rem 1rem; }
}

/* ── VERY SMALL (≤ 360px) ── */
@media (max-width: 360px) {
  .cert-3d { width: 130px; height: 92px; }
  .hero-title-main { font-size: 2.2rem; }
  .hero-stat { font-size: 0.68rem; padding: 0.25rem 0.5rem; }
}
</style>
...
with st.sidebar:
    ...
    st.markdown("""
    <div style='font-size:0.76rem;color:rgba(253,246,227,0.36);line-height:2.1;'>
    ...
    <div class="step-card">
      <div class="step-header">
        ...
        <div>
          ...
          <div class="step-desc">Tell AutoCertify which column holds which data</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
if send_clicked:
    ...
    for index, row in data.iterrows():
        ...
        status_text.markdown(
            ...
            f' &nbsp;·&nbsp; <b style="color:#f87171">❌ {failed}</b>'
            f'</div>',
            unsafe_allow_html=True
        )
